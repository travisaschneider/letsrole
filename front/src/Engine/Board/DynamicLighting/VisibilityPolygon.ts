...
VisibilityPolygon.compute = function (position, segments) {
  ...
  for (let i = 0; i < sorted.length; ) {
    ...
    if (extend) {
      polygon.push(vertex);
      const cur = VisibilityPolygon.intersectLines(
        bounded[heap[0]][0],
        bounded[heap[0]][1],
        position,
        vertex
  ...
  for (let i = 0; i < segments.length; ++i) {
    ...
    for (let j = 0; j < viewport.length; ++j) {
      ...
      if (
        ...
        const intersect = VisibilityPolygon.intersectLines(
          segments[i][0],
          segments[i][1],
          viewport[j],
          viewport[k]
...
VisibilityPolygon.inViewport = function (
  position,
  viewportMinCorner,
  viewportMaxCorner
) {
  if (position[0] < viewportMinCorner[0] - VisibilityPolygon.epsilon())
    return false;
  if (position[1] < viewportMinCorner[1] - VisibilityPolygon.epsilon())
    return false;
  if (position[0] > viewportMaxCorner[0] + VisibilityPolygon.epsilon())
    return false;
  if (position[1] > viewportMaxCorner[1] + VisibilityPolygon.epsilon())
    return false;
  return true;
};
  ...
  for (let i = 0; i < polygon.length; ++i) {
    ...
    if (
      ...
      if (VisibilityPolygon.equal(intersect, polygon[i])) {
        if (VisibilityPolygon.angle2(position, edge, polygon[j]) < 180)
          ++parity;
      } else if (VisibilityPolygon.equal(intersect, polygon[j])) {
        if (VisibilityPolygon.angle2(position, edge, polygon[i]) < 180)
          ++parity;
      } else {
        ++parity;
      }
...
VisibilityPolygon.insert = function (
  index,
  heap,
  position,
  segments,
...
VisibilityPolygon.lessThan = function (
  index1,
  index2,
  position,
  segments,
  ...
  const inter1 = VisibilityPolygon.intersectLines(
    segments[index1][0],
    segments[index1][1],
    position,
    destination
  );
  const inter2 = VisibilityPolygon.intersectLines(
    segments[index2][0],
    segments[index2][1],
    position,
    destination
...
VisibilityPolygon.doLineSegmentsIntersect = function (
  x1,
  y1,
  x2,
  y2,
